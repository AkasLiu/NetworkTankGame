...
function sceneManager:start(  )
    ...
    NetworkManager.Instance:RegisterProto(ProtocolID.ExitGame,packFunction(self,self.playerExit))
end

function sceneManager:playerExit( param )
	local exitGameProtocol = Common.Protocol.ExitGameProtocol()
	exitGameProtocol:Decode(param)
	if next(tanks) ~= nil then
		for k,v in pairs(tanks) do
			if k == exitGameProtocol.Role_id then
...
function sceneManager:exit(  )
	...
	local exitGameProtocol = Common.Protocol.ExitGameProtocol(userData.id)
    ...
    NetworkManager.Instance:UnregisterProto(ProtocolID.ExitGame,packFunction(self,self.playerExit))
end
...
function sceneManager:revive( param )
	...
	local myTransform = Common.MyTransform(9.0, -1.0, -7.0, 0.0, 0.0, 0.0)
	self:generateTank(reviveProtocol.Role_id,myTransform)
end
